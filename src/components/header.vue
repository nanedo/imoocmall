<template>
  ...
  <Modal :mdShow="loginModalFlag" @close="closeLoginModal" mdTitle="Login in" :hideClose="hideClose" :clickBgClose="clickBgClose">
      ...
      <a href="javascript:;" class="btn-login" @click="login" slot="btnGroup">登  录</a>
  </Modal>
  ...
  <Register :isShow="registerModalFlag" @close="closeRegisterModal"></Register>
</header>
</template>
<script>
...
export default {
  ...
  mounted () {
    this.checkLogin()
    // 未登录状态
    eventBus.$on('unLogin', (doSomethinAfterLogin, popConfig) => {
